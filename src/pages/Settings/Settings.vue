<template>
  <div>
    ...
    <b-container fluid id="settings" class="my-2">
      ...
      <b-form-group :invalid-feedback="pokeballError">
        <b-form-file
          class="mb-2"
          ...
          @input="setPokeball"
          :state="pokeballError ? false : null"
        ></b-form-file>
      </b-form-group>
...
export default {
  mounted() {
    this.$store.subscribe((mutation, state) => {
      window.ipcRenderer.send("vuex-mutation", mutation);
  ...
  methods: {
    close() {
      window.ipcRenderer.send("settings-close");
    },
    minimize() {
      window.ipcRenderer.send("settings-minimize");
    },
    setPokeball(value) {
      this.pokeballError = null;
      if (
        ...
        this.pokeballError = this.$t("unsupported_format");
      }
    },
    setToDefault() {
